...
ctrl_t BSplineGeometry::evaluate(knot_t const& x, size_t tid) 
{
	...
	std::vector<size_t> bstack(n_kdims + 1);
	bstack[0] = 1;
...
int main() 
{
...

	{
		...
		std::vector<knot_t> knots{_knots};
...
/*	
	{
		...
		std::vector<size_t> degrees{0};
		knot_t _knots{0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1};
		std::vector<knot_t> knots{_knots};
		std::vector<ctrl_t> control_points{{0, 1}, {0.125, 2}, {0.25, 4}, {0.375, 8}, {0.5, 16}, {0.625, 32}, {0.75, 64}, {0.875, 128}};
		BSplineGeometry spline = BSplineGeometry(1, 2, 1, degrees, knots, control_points);
...

	{
		...
		auto spline = BSplineGeometry(2, 2, 1, degrees, knots, control_points);
...

	{
		...
		auto spline = BSplineGeometry(3, 3, 1, degrees, knots, control_points);
	...
	
	{
		...
		auto spline = BSplineGeometry(1, 1, 1, degrees, knots, control_points);
